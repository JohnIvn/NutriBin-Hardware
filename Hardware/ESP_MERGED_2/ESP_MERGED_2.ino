...
#define TRIGGER_1_PIN      35   // HIGH = sort to 0°
#define TRIGGER_2_PIN      34   // HIGH = sort to 180°  
...
bool   trigger1Active        = false; // HIGH = move servo1 to 0°
bool   trigger2Active        = false; // HIGH = move servo1 to 180°
bool   trigger1Wireless      = false; // Wireless classification trigger (persists)
bool   trigger2Wireless      = false; // Wireless classification trigger (persists)
float  distanceCm            = 0.0;
int    servo1TargetAngle     = 0;
int    servo1CurrentAngle    = 90; // Track actual servo 1 angle for reliable closing
int    servo3CycleCount      = 0;  // Track servo 3 dump cycles
...
// =====================================================================
// OFFLINE AP + LOG BUFFER
// Device will run as an access point and host a small web UI showing
// recent sensor payloads. Uploads to the remote backend are disabled
// but the POST code is retained for future re-enabling.
// =====================================================================
...
// --- Web Log Buffer (for Serial/Event logging) ---
const int WEB_LOG_SIZE = 30;
String webLogs[WEB_LOG_SIZE];
int webLogIndex = 0;
int webLogCount = 0;

void addWebLog(String msg) {
  Serial.println(msg); // Keep Serial mirror
  webLogs[webLogIndex] = msg;
...
void setup() {
  ...
  setupServos();
  servo1.write(90);
  servo1CurrentAngle = 90; // Track initial position
  servo2.write(90);
  servo3.write(180);
  servo3CycleCount = 0; // Initialize cycle counter
...
void loop() {
  ...
  switch (currentState) {
    ...
    case STATE_TRIGGER_ACTIVE:
      if (!trigger1Active && !trigger2Active) {
        addWebLog("Trigger lost, returning to wait");
        resetToIdle();
        break;
      }
      // For wireless triggers, skip ultrasonic and go straight to reed check
      if (trigger1Wireless || trigger2Wireless) {
        addWebLog("Wireless trigger detected - skipping ultrasonic, checking reed...");
        currentState = STATE_OBJECT_DETECTED;
        stateStartTime = now;
        break;
      }
      // For hardware triggers, monitor ultrasonic for object detection
      if (now - lastDistanceRead >= DISTANCE_READ_INTERVAL) {
        ...
        if (distanceCm >= DETECTION_DISTANCE_MIN && distanceCm <= DETECTION_DISTANCE_MAX) {
          addWebLog("Object detected: " + String(distanceCm) + " cm");
          currentState   = STATE_OBJECT_DETECTED;
          stateStartTime = now;
        }
      }
    ...
    case STATE_REED_CHECK:
      if (reedSwitchOpen) {
        addWebLog("ERROR: Reed open — lid is open, aborting");
        currentState   = STATE_REED_OPEN_ERROR;
        stateStartTime = now;
        uploadData();
      } else {
        addWebLog("Reed closed — starting sequence");
    ...
    case STATE_REED_OPEN_ERROR:
      if (now - stateStartTime >= 5000) {
        addWebLog("Resetting after reed error...");
        resetToIdle();
      }
      break;

    case STATE_SERVO_1_SORTING: {
      addWebLog("Servo 1: 90° -> " + String(servo1TargetAngle) + "°");
      ...
      servo1CurrentAngle = angle; // Track actual position
      addWebLog("Servo 1 sorted");
    ...
    case STATE_SERVO_1_CLOSING:
      if (now - stateStartTime >= SERVO_1_CLOSE_DELAY) {
        addWebLog("Servo 1: returning to 90°");
        int angle = servo1CurrentAngle; // Use tracked angle, not servo.read()
        ...
        servo1CurrentAngle = 90; // Update tracked position
        // Check if pin 35 triggered: skip grinder & servo 2 if true
        if (trigger1Active) {
          addWebLog("Pin 35 active: skipping grinder & servo 2, proceeding to dump");
          servo3CycleCount = 0; // Reset cycle counter
          currentState = STATE_SERVO_3_DUMPING;
        } else {
          addWebLog("Servo 1 closed — activating grinder...");
    ...
    case STATE_SSR_GRINDING:
      if (now - stateStartTime >= SSR_GRINDER_TIME) {
        ...
        addWebLog("Grinder stopped — opening gate (Servo 2)...");
    ...
    case STATE_SERVO_2_OPENING: {
      addWebLog("Servo 2 opening: 90° -> 180°");
      for (int angle = 90; angle <= 180; angle += 3) {
        servo2.write(angle);
        delay(20);
      }
      servo2.write(180); // Ensure full position
      addWebLog("Servo 2 opened to 180°");
      delay(500);
    ...
    case STATE_SERVO_2_CLOSING: {
      addWebLog("Servo 2 closing: 180° -> 90°");
      for (int angle = 180; angle >= 90; angle -= 3) {
        servo2.write(angle);
        delay(20);
      }
      servo2.write(90); // Ensure full position
      addWebLog("Servo 2 returned to 90° — final dump (Servo 3)...");
      servo3CycleCount = 0; // Reset cycle counter
      currentState = STATE_SERVO_3_DUMPING;
      break;
    }
    ...
    case STATE_SERVO_3_DUMPING: {
      if (servo3CycleCount < 5) {
        addWebLog("Servo 3 dump cycle " + String(servo3CycleCount + 1) + "/5");
        // Dump: 180° -> 0°
        ...
        addWebLog("Servo 3: 5 cycles COMPLETE");
        sequencesCompleted++;
...
void connectWiFi() {
  if (apStarted) return;
  Serial.println("\n--- Starting offline Access Point (AP mode) ---");
  const char* ap_ssid = "NutriBin-AP";
  const char* ap_pass = ""; // open AP (no password); set a password if desired
  WiFi.mode(WIFI_AP);
  bool ok = WiFi.softAP(ap_ssid, ap_pass);
  apStarted = true;
  IPAddress ip = WiFi.softAPIP();
  Serial.println(String("AP started: ") + ap_ssid + " IP: " + ip.toString());
}
...
// =====================================================================
bool uploadData() {
  // Offline mode: build payload, log locally and to Serial, but do NOT send.
  Serial.println("\n--- Building local payload (offline mode) ---");
  ...
  // Push to in-memory ring buffer for the AP web UI
  logBuffer[logIndex] = payload;
  logIndex = (logIndex + 1) % LOG_BUFFER_SIZE;
  if (logCount < LOG_BUFFER_SIZE) logCount++;

  // Mark that uploads are disabled while keeping the remote code intact
  lastUploadSuccess = false;
  lastErrorMessage = "Upload disabled (offline)";
  return false;
}
...
void checkExternalTriggers() {
  // Read hardware pins
  bool hw_trigger1 = digitalRead(TRIGGER_1_PIN);
  bool hw_trigger2 = digitalRead(TRIGGER_2_PIN);
  
  // Combine hardware pins with wireless triggers (wireless takes precedence)
  trigger1Active = hw_trigger1 || trigger1Wireless;
  trigger2Active = hw_trigger2 || trigger2Wireless;
}
...
void resetToIdle() {
  servo1.write(90); servo1CurrentAngle = 90;
  servo2.write(90);
  servo3.write(180);
  digitalWrite(SSR_RELAY_PIN, LOW);
  trigger1Active = false; trigger2Active = false;
  trigger1Wireless = false; trigger2Wireless = false;  // Clear wireless triggers
  servo1TargetAngle = 0;
  servo3CycleCount = 0;
  currentState = STATE_WAITING_TRIGGER;
  addWebLog("System reset — WAITING FOR TRIGGER");
}
...
void handleRoot() {
  ...
  // System Logs (Serial mirror)
  html += "<h2>System & Servo Logs</h2>";
  html += "<div style='background:#000;color:#0f0;padding:10px;border:1px solid #333;font-family:monospace;max-height:300px;overflow-y:auto;'>";
  if (webLogCount == 0) {
    html += "Waiting for events...";
  } else {
    // Show in reverse order (newest first)
    for (int i = 0; i < webLogCount; i++) {
      int idx = (webLogIndex - 1 - i + WEB_LOG_SIZE) % WEB_LOG_SIZE;
      html += "<div>> " + webLogs[idx] + "</div>";
    }
  }
  html += "</div>";

  // Recent payload logs (in-memory)
  html += "<h2>Recent Payloads</h2>";
  html += "<table><tr><th>#</th><th>Payload (JSON)</th></tr>";
  if (logCount == 0) {
    html += "<tr><td colspan='2'>No logs available</td></tr>";
  } else {
    int start = (logIndex - logCount + LOG_BUFFER_SIZE) % LOG_BUFFER_SIZE;
    for (int i = 0; i < logCount; i++) {
      int idx = (start + i) % LOG_BUFFER_SIZE;
      html += "<tr><td>" + String(i + 1) + "</td><td><pre style='white-space:pre-wrap;max-width:800px;'>" + logBuffer[idx] + "</pre></td></tr>";
    }
  }
...
// =====================================================================
// CLASSIFICATION HANDLER (from ML classifier)
// =====================================================================
void handleClassification() {
  ...
  if (error) {
    addWebLog("Classification: JSON parse error - " + String(error.c_str()));
    server.send(400, "application/json", "{\"error\":\"Invalid JSON\"}");
    return;
  }

  // Extract classification and trigger
  String classification = doc["classification"] | "";
  int trigger = doc["trigger"] | 0;
  float probability = doc["probability"] | 0.0;

  addWebLog("\n>>> WIRELESS CLASSIFICATION RECEIVED <<<");
  addWebLog("Classification: " + classification + " (prob: " + String(probability, 2) + ")");
  addWebLog("Trigger: " + String(trigger));

  // Trigger the appropriate pin (1=pin 35 for paper, 2=pin 34 for leaves)
  if (trigger == 1) {
    addWebLog("Triggering PIN 35 (TRIGGER_1_PIN) — Paper detected");
    trigger1Wireless = true;
    trigger2Wireless = false;
  } else if (trigger == 2) {
    addWebLog("Triggering PIN 34 (TRIGGER_2_PIN) — Leaves/Organic detected");
    trigger1Wireless = false;
    trigger2Wireless = true;
  } else {
    addWebLog("Unknown trigger value: " + String(trigger));
    server.send(400, "application/json", "{\"error\":\"Invalid trigger value\"}");
    return;
  }

  // Send success response
  server.send(200, "application/json", "{\"status\":\"ok\",\"message\":\"Classification received and trigger activated\"}");
}